...
\{
    ...
    \textcolor{keywordtype}{unsigned} period =
        \textcolor{keyword}{static\_cast<}\textcolor{keywordtype}{unsigned}\textcolor{keyword}{>}(round((1.0 / freq) * pow(10.0, 9.0)));
    ...
    \{
        ...
        \textcolor{comment}{// printf("%ld ; %ld ; ", sleep\_duration.tv\_nsec,}
        \textcolor{comment}{// sleep\_duration\_diff.count()); printf("sleeping time is %ld  \(\backslash\)n",}
        \textcolor{comment}{// sleep\_duration.tv\_nsec);}

        mid = my\_clock::now();
        sleep\_duration.tv\_nsec =
            (clock\_period - sleep\_duration\_diff - (mid - start)).count();

        nanosleep(&sleep\_duration, &out\_sleep);  \textcolor{comment}{// microseconds, so in Ghz}

        stop = my\_clock::now();
        sleep\_duration\_diff = my\_clock::duration(
            (\textcolor{keywordtype}{unsigned})((stop - mid) -
                       my\_clock::duration(sleep\_duration.tv\_nsec))
                .count());
    \}
